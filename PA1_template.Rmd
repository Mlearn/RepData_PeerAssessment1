...
The format of dat is **`r class(dat)`**
...
median(steps_day)
```
Of the total number of steps taken per day, the _mean_ is **`r mean(steps_day)`**
...
The No.**`r which.max(steps_interval)`** 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps.
...
The _tatal number_ of mission values in the dataset is **`r sum(is.na(dat$steps))`**
