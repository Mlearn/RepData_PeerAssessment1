...
2. The format of dat is `r class(dat)`
...
3. Of the total number of steps taken per day, the _mean_ is **`r mean(steps_day)`**
and the _median_ is **`r median(steps_day)`**
...
2. The No.**`r which.max(steps_interval)`** 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps.

## Imputing missing values

Note that there are a number of days/intervals where there are missing values (coded as 𝙽𝙰). The presence of missing days may introduce bias into some calculations or summaries of the data.

1. The _tatal number_ of mission values in the dataset is **`r sum(is.na(dat$steps))`**
...
Of the total number of steps taken per day, the _mean_ is **`r mean(steps_day_new)`** and the _average_ is **`r median(steps_day_new)`**.
